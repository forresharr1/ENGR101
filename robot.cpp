...
void whiteLineFolow (vector<PixalLocation>WHITEPixalFoundVector){
  ...
  for (int whitepixal = 0; whitepixal<cameraView.width; whitepixal++)
  {
    ...
    if(tempRow > centerLine){leftWhitePixals = leftWhitePixals + 20 ; }
    // 	if the white is on the right then adds one to the rightwhitePixals
    if(tempRow < centerLine){rightwhitePixals = rightwhitePixals + 20 + 20;}
  }
...
//   /100 slows it down so it can see it better
  vLeft = whitevLeft / 10;//- redvLeft   whitevLeft   
  vRight = whitevRight / 10 ;//- redvRight  whitevRight
...
int main(){
  ...
  while(1){
   ...
   //these will hold the raw color of each pixal
    ...
    cout<<"vector size "<<WHITEPixalFoundVector.size()<<endl;// outputs the vector size 
